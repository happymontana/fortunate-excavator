...
| Long gaps and reconnections | ordered timestamps per person, gap threshold | Threshold is a judgement call, not a fact. Default: a gap ≥ 12 months. A *reconnection* is that gap closed by **sustained** contact — ≥ N messages within M days — never one stray message ([`build-guide.md`](build-guide.md) §4). Defaults, set with the operator 2026-09-22: **N = 3 within M = 30 days**, and the gap length itself is freely configurable. All three are recorded in the manifest. **A gap that overlaps an archive-wide coverage hole is not a gap**: it is missing data, such as a device whose messages never synced. Detect such holes from whole-archive monthly volume, report them once as coverage, and never as a per-person gap or reconnection. Make both configurable and *show* them in the report. |
